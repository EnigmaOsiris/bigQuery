...
class BigQuery(object):
    ...
    def __init__(self, cred_file, project, verbose=True, name='Process'):
        """
    ...
    def run_query(self,
        ...
        if destination_table is not None:
            ...
            # NOTE: This saved to the class before, is that still needed?
            self.dataset = self.client.dataset(destination_dataset)
            job.destination = self.dataset.table(destination_table)
            # NOTE: This might need to be parameterized
    ...
    def process_results(self, results, max_results=1000, fetch_all=True):
        ...
        time_init = None
        time_finish = None
        """print("=> Starting fetching next page :  %s at  %s" %
              (self.name, time.ctime(time.time())))"""
        print("=> Starting fetching next page :  %s at  %s" % (self.name,
                                                               datetime.now()))
        ...
        delay_time = time_finish - time_init
        duration_in_s = delay_time.total_seconds()
        days = divmod(duration_in_s, 86400)
        hours = divmod(days[1], 3600)
        minutes = divmod(hours[1], 60)
        seconds = divmod(minutes[1], 1)
        print("=> Time Elpased: %s Minutes %s Seconds" % (minutes, seconds))
        ...
        while fetch_all:
            ...
            if rows.next_page_token is None:  # Stop looping on the last page of results
                break
            rows = results.fetch_data(
                max_results=max_results, page_token=rows.next_page_token)
            ...
            minutes = divmod(hours[1], 60)
            seconds = divmod(minutes[1], 1)
            print("=> Time Elpased: %s Minutes %s Seconds" % (minutes[0],
                                                              seconds[0]))
        ...
        if self.verbose:
            print("Processed " + str(tot) + " results")
    ...
    def export_table(self,
        ...
        table_ref = self.client.dataset(
            dataset, project=project).table(table_id)
        if self.verbose:
            print("Starting load of {} to {} as {}".format(
                table_id, destination_url, job_id))

        extract_job = self.client.extract_table_to_storage(
            job_id, table_ref, destination_url)
        extract_job.destination_format = "NEWLINE_DELIMITED_JSON"
        extract_job._build_resource()
        extract_job.begin()
        result = extract_job.result().state
        if self.verbose:
            print("Job {} is finished with a status of {}".format(
                job_id, result))
